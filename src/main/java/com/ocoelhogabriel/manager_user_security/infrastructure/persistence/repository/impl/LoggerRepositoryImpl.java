 ...
 */
public record LoggerRepositoryImpl(
        LoggerJpaRepository loggerJpaRepository,
        LoggerMapper loggerMapper) implements LoggerRepository {

    public LoggerRepositoryImpl(@Qualifier("loggerRepositoryAdapter") LoggerJpaRepository loggerJpaRepository, LoggerMapper loggerMapper) {
    ...
    public List<Logger> findAll() {
        return loggerJpaRepository.findAll().stream().map(loggerMapper::toDomain).collect(Collectors.toList());
    }

    @Override
    public List<Logger> findByType(LoggerType type) {
        return loggerJpaRepository.findByType(type).stream().map(loggerMapper::toDomain).collect(Collectors.toList());
    }

    @Override
    public List<Logger> findBySerialNumber(String serialNumber) {
        return loggerJpaRepository.findBySerialNumber(serialNumber).stream().map(loggerMapper::toDomain).collect(Collectors.toList());
    }

    @Override
    public List<Logger> findByTimestampBetween(LocalDateTime startTime, LocalDateTime endTime) {
        return loggerJpaRepository.findByTimestampBetween(startTime, endTime).stream().map(loggerMapper::toDomain).collect(Collectors.toList());
    }
    ...
    public List<Logger> findLatestLogs(int limit) {
        return loggerJpaRepository.findTop100ByOrderByTimestampDesc().stream().limit(limit).map(loggerMapper::toDomain).collect(Collectors.toList());
    }
}
